# its not perfect , like me :)))
#blue print = core of code
...
import requests
class Blockchain():
    ''' defines a block on one machine '''
    def __init__(self):
        self.chain = []
        self.current_trxs = []
        self.nodes = set()
...
def mine():
    ...
    res = { 'message' : 'new block created' , 'index' : block['index'] , 'trxs' : block['trxs'] , 'proof' : block['proof'] , 'previous_hash' : block['previous_hash'],
    }
...
def register_node():
    ...
    res = {'message' : 'nodes added' , 'total nodes' : list(blockchain.nodes)}  
